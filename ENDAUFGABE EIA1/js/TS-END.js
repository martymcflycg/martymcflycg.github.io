//Spieler-Karten generieren//
let Karte1 = {
    Wert: "1",
    Typ: "Rot",
    Reihenfolge: 1,
    Rückseite: "normal",
};
let Karte2 = {
    Wert: "2",
    Typ: "Rot",
    Reihenfolge: 2,
    Rückseite: "normal",
};
let Karte3 = {
    Wert: "3",
    Typ: "Rot",
    Reihenfolge: 3,
    Rückseite: "normal",
};
let Karte4 = {
    Wert: "4",
    Typ: "Rot",
    Reihenfolge: 4,
    Rückseite: "normal",
};
let Karte5 = {
    Wert: "5",
    Typ: "Rot",
    Reihenfolge: 5,
    Rückseite: "normal",
};
let Karte6 = {
    Wert: "6",
    Typ: "Rot",
    Reihenfolge: 6,
    Rückseite: "normal",
};
let Karte7 = {
    Wert: "7",
    Typ: "Rot",
    Reihenfolge: 7,
    Rückseite: "normal",
};
let Karte8 = {
    Wert: "8",
    Typ: "Rot",
    Reihenfolge: 8,
    Rückseite: "normal",
};
let Karte9 = {
    Wert: "9",
    Typ: "Rot",
    Reihenfolge: 9,
    Rückseite: "normal",
};
let Karte10 = {
    Wert: "10",
    Typ: "Rot",
    Reihenfolge: 10,
    Rückseite: "normal",
};
let Karte11 = {
    Wert: "1",
    Typ: "Gelb",
    Reihenfolge: 11,
    Rückseite: "normal",
};
let Karte12 = {
    Wert: "2",
    Typ: "Gelb",
    Reihenfolge: 12,
    Rückseite: "normal",
};
let Karte13 = {
    Wert: "3",
    Typ: "Gelb",
    Reihenfolge: 13,
    Rückseite: "normal",
};
let Karte14 = {
    Wert: "4",
    Typ: "Gelb",
    Reihenfolge: 14,
    Rückseite: "normal",
};
let Karte15 = {
    Wert: "5",
    Typ: "Gelb",
    Reihenfolge: 15,
    Rückseite: "normal",
};
let Karte16 = {
    Wert: "6",
    Typ: "Gelb",
    Reihenfolge: 16,
    Rückseite: "normal",
};
let Karte17 = {
    Wert: "7",
    Typ: "Gelb",
    Reihenfolge: 17,
    Rückseite: "normal",
};
let Karte18 = {
    Wert: "8",
    Typ: "Gelb",
    Reihenfolge: 18,
    Rückseite: "normal",
};
let Karte19 = {
    Wert: "9",
    Typ: "Gelb",
    Reihenfolge: 19,
    Rückseite: "normal",
};
let Karte20 = {
    Wert: "10",
    Typ: "Gelb",
    Reihenfolge: 20,
    Rückseite: "normal",
};
let Karte21 = {
    Wert: "1",
    Typ: "Blau",
    Reihenfolge: 21,
    Rückseite: "normal",
};
let Karte22 = {
    Wert: "2",
    Typ: "Blau",
    Reihenfolge: 22,
    Rückseite: "normal",
};
let Karte23 = {
    Wert: "3",
    Typ: "Blau",
    Reihenfolge: 23,
    Rückseite: "normal",
};
let Karte24 = {
    Wert: "4",
    Typ: "Blau",
    Reihenfolge: 24,
    Rückseite: "normal",
};
let Karte25 = {
    Wert: "5",
    Typ: "Blau",
    Reihenfolge: 25,
    Rückseite: "normal",
};
let Karte26 = {
    Wert: "6",
    Typ: "Blau",
    Reihenfolge: 26,
    Rückseite: "normal",
};
let Karte27 = {
    Wert: "7",
    Typ: "Blau",
    Reihenfolge: 27,
    Rückseite: "normal",
};
let Karte28 = {
    Wert: "8",
    Typ: "Blau",
    Reihenfolge: 28,
    Rückseite: "normal",
};
let Karte29 = {
    Wert: "9",
    Typ: "Blau",
    Reihenfolge: 29,
    Rückseite: "normal",
};
let Karte30 = {
    Wert: "10",
    Typ: "Blau",
    Reihenfolge: 30,
    Rückseite: "normal",
};
let Karte31 = {
    Wert: "1",
    Typ: "Gruen",
    Reihenfolge: 31,
    Rückseite: "normal",
};
let Karte32 = {
    Wert: "2",
    Typ: "Gruen",
    Reihenfolge: 32,
    Rückseite: "normal",
};
let Karte33 = {
    Wert: "3",
    Typ: "Gruen",
    Reihenfolge: 33,
    Rückseite: "normal",
};
let Karte34 = {
    Wert: "4",
    Typ: "Gruen",
    Reihenfolge: 34,
    Rückseite: "normal",
};
let Karte35 = {
    Wert: "5",
    Typ: "Gruen",
    Reihenfolge: 35,
    Rückseite: "normal",
};
let Karte36 = {
    Wert: "6",
    Typ: "Gruen",
    Reihenfolge: 36,
    Rückseite: "normal",
};
let Karte37 = {
    Wert: "7",
    Typ: "Gruen",
    Reihenfolge: 37,
    Rückseite: "normal",
};
let Karte38 = {
    Wert: "8",
    Typ: "Gruen",
    Reihenfolge: 38,
    Rückseite: "normal",
};
let Karte39 = {
    Wert: "9",
    Typ: "Gruen",
    Reihenfolge: 39,
    Rückseite: "normal",
};
let Karte40 = {
    Wert: "10",
    Typ: "Gruen",
    Reihenfolge: 40,
    Rückseite: "normal",
};
//Computer-Karten generieren//
let Karte41 = {
    Wert: "1",
    Typ: "Rot",
    Reihenfolge: 1,
    Rückseite: "anders",
};
let Karte42 = {
    Wert: "2",
    Typ: "Rot",
    Reihenfolge: 2,
    Rückseite: "anders",
};
let Karte43 = {
    Wert: "3",
    Typ: "Rot",
    Reihenfolge: 3,
    Rückseite: "anders",
};
let Karte44 = {
    Wert: "4",
    Typ: "Rot",
    Reihenfolge: 4,
    Rückseite: "anders",
};
let Karte45 = {
    Wert: "5",
    Typ: "Rot",
    Reihenfolge: 5,
    Rückseite: "anders",
};
let Karte46 = {
    Wert: "6",
    Typ: "Rot",
    Reihenfolge: 6,
    Rückseite: "anders",
};
let Karte47 = {
    Wert: "7",
    Typ: "Rot",
    Reihenfolge: 7,
    Rückseite: "anders",
};
let Karte48 = {
    Wert: "8",
    Typ: "Rot",
    Reihenfolge: 8,
    Rückseite: "anders",
};
let Karte49 = {
    Wert: "9",
    Typ: "Rot",
    Reihenfolge: 9,
    Rückseite: "anders",
};
let Karte50 = {
    Wert: "10",
    Typ: "Rot",
    Reihenfolge: 10,
    Rückseite: "anders",
};
let Karte51 = {
    Wert: "1",
    Typ: "Gelb",
    Reihenfolge: 11,
    Rückseite: "anders",
};
let Karte52 = {
    Wert: "2",
    Typ: "Gelb",
    Reihenfolge: 12,
    Rückseite: "anders",
};
let Karte53 = {
    Wert: "3",
    Typ: "Gelb",
    Reihenfolge: 13,
    Rückseite: "anders",
};
let Karte54 = {
    Wert: "4",
    Typ: "Gelb",
    Reihenfolge: 14,
    Rückseite: "anders",
};
let Karte55 = {
    Wert: "5",
    Typ: "Gelb",
    Reihenfolge: 15,
    Rückseite: "anders",
};
let Karte56 = {
    Wert: "6",
    Typ: "Gelb",
    Reihenfolge: 16,
    Rückseite: "anders",
};
let Karte57 = {
    Wert: "7",
    Typ: "Gelb",
    Reihenfolge: 17,
    Rückseite: "anders",
};
let Karte58 = {
    Wert: "8",
    Typ: "Gelb",
    Reihenfolge: 18,
    Rückseite: "anders",
};
let Karte59 = {
    Wert: "9",
    Typ: "Gelb",
    Reihenfolge: 19,
    Rückseite: "anders",
};
let Karte60 = {
    Wert: "10",
    Typ: "Gelb",
    Reihenfolge: 20,
    Rückseite: "anders",
};
let Karte61 = {
    Wert: "1",
    Typ: "Blau",
    Reihenfolge: 21,
    Rückseite: "anders",
};
let Karte62 = {
    Wert: "2",
    Typ: "Blau",
    Reihenfolge: 22,
    Rückseite: "anders",
};
let Karte63 = {
    Wert: "3",
    Typ: "Blau",
    Reihenfolge: 23,
    Rückseite: "anders",
};
let Karte64 = {
    Wert: "4",
    Typ: "Blau",
    Reihenfolge: 24,
    Rückseite: "anders",
};
let Karte65 = {
    Wert: "5",
    Typ: "Blau",
    Reihenfolge: 25,
    Rückseite: "anders",
};
let Karte66 = {
    Wert: "6",
    Typ: "Blau",
    Reihenfolge: 26,
    Rückseite: "anders",
};
let Karte67 = {
    Wert: "7",
    Typ: "Blau",
    Reihenfolge: 27,
    Rückseite: "anders",
};
let Karte68 = {
    Wert: "8",
    Typ: "Blau",
    Reihenfolge: 28,
    Rückseite: "anders",
};
let Karte69 = {
    Wert: "9",
    Typ: "Blau",
    Reihenfolge: 29,
    Rückseite: "anders",
};
let Karte70 = {
    Wert: "10",
    Typ: "Blau",
    Reihenfolge: 30,
    Rückseite: "anders",
};
let Karte71 = {
    Wert: "1",
    Typ: "Gruen",
    Reihenfolge: 31,
    Rückseite: "anders",
};
let Karte72 = {
    Wert: "2",
    Typ: "Gruen",
    Reihenfolge: 32,
    Rückseite: "anders",
};
let Karte73 = {
    Wert: "3",
    Typ: "Gruen",
    Reihenfolge: 33,
    Rückseite: "anders",
};
let Karte74 = {
    Wert: "4",
    Typ: "Gruen",
    Reihenfolge: 34,
    Rückseite: "anders",
};
let Karte75 = {
    Wert: "5",
    Typ: "Gruen",
    Reihenfolge: 35,
    Rückseite: "anders",
};
let Karte76 = {
    Wert: "6",
    Typ: "Gruen",
    Reihenfolge: 36,
    Rückseite: "anders",
};
let Karte77 = {
    Wert: "7",
    Typ: "Gruen",
    Reihenfolge: 37,
    Rückseite: "anders",
};
let Karte78 = {
    Wert: "8",
    Typ: "Gruen",
    Reihenfolge: 38,
    Rückseite: "anders",
};
let Karte79 = {
    Wert: "9",
    Typ: "Gruen",
    Reihenfolge: 39,
    Rückseite: "anders",
};
let Karte80 = {
    Wert: "10",
    Typ: "Gruen",
    Reihenfolge: 40,
    Rückseite: "anders",
};
//Array für Hauptstapel wird erstellt
let kartenStapel = [Karte1, Karte2, Karte3, Karte4, Karte5, Karte6, Karte7, Karte8,
    Karte9, Karte10, Karte11, Karte12, Karte13, Karte14, Karte15, Karte16, Karte17, Karte18,
    Karte19, Karte20, Karte21, Karte22, Karte23, Karte24, Karte25, Karte26, Karte27, Karte28,
    Karte29, Karte30, Karte31, Karte32, Karte33, Karte34, Karte35, Karte36, Karte37, Karte38, Karte39, Karte40];
//Array für Computer-Karten wird erstellt
let gegnerKarten = [Karte41, Karte42, Karte43, Karte44, Karte45, Karte46, Karte47, Karte48,
    Karte49, Karte50, Karte51, Karte52, Karte53, Karte54, Karte55, Karte56, Karte57, Karte58,
    Karte59, Karte60, Karte61, Karte62, Karte63, Karte6, Karte65, Karte66, Karte67, Karte68,
    Karte69, Karte70, Karte71, Karte72, Karte73, Karte74, Karte75, Karte76, Karte77, Karte78, Karte79,];
//Handkarten (Decks)
let kartenHand = [];
let kartenHand1 = [];
let compHand = [];
let x;
let v;
let y;
let y6;
let ablagestapel = [];
//writehtml1 Funktion
function writeHtml(_y) {
    let prodElement2 = document.createElement('div');
    document.getElementById("kartenHand").appendChild(prodElement2);
    let karte = `<div class="${kartenHand[_y].Typ}" id="${kartenHand[_y].Reihenfolge}">${kartenHand[_y].Wert}</div>`;
    prodElement2.innerHTML = karte;
}
//writehtml2 Funktion
function writeHtml2(_y1) {
    let prodElement3 = document.createElement('div');
    document.getElementById("Computer").appendChild(prodElement3);
    let kartennn = `<div class="${compHand[_y1].Typ}" id="${compHand[_y1].Reihenfolge}">${compHand[_y1].Wert}</div>`;
    prodElement3.innerHTML = kartennn;
}
//Onload-Funktionen
window.onload = function () {
    var randomCard = kartenStapel[Math.floor((Math.random() * kartenStapel.length))];
    ablagestapel = [randomCard];
    for (let i = kartenHand.length; i > 0; i--) {
        document.getElementById("kartenHand").innerHTML = "";
        let pprodElement = document.createElement('div');
        document.getElementById("ablagestapel").appendChild(pprodElement);
        let ablegekarte = `<div class="${randomCard.Typ}" id="${randomCard.Reihenfolge}"> ${randomCard.Wert}</div>`;
        pprodElement.innerHTML = ablegekarte;
        writeHtml(i);
        writeHtml2(i);
        function Onload2() {
            var randomCard2 = gegnerKarten[Math.floor((Math.random() * gegnerKarten.length))];
            ablagestapel = [randomCard2];
            console.log("bhjbdhjbdjch");
            for (let i = compHand.length; i > 0; i++) {
                document.getElementById("Computer").innerHTML = "";
                let pprodElement = document.createElement('div');
                document.getElementById("ablagestapel").appendChild(pprodElement);
                let ablegekarten = `<div class="${randomCard2.Typ}" id="${randomCard2.Reihenfolge}"> ${randomCard2.Wert}</div>`;
                pprodElement.innerHTML = ablegekarten;
            }
        }
    }
    Onload2();
};
//Karten austeilen Funktion
function Kartenausteilen() {
    for (y = 0; y; y++) {
        x = Math.floor((Math.random() * kartenStapel.length));
        v = Math.floor((Math.random() * kartenStapel.length));
        kartenHand.push(kartenStapel[x]);
        compHand.push(gegnerKarten[v]);
        writeHtml(y);
        writeHtml2(y);
        kartenStapel.splice(x, 1);
    }
}
Kartenausteilen();
//Karte ziehen
document.getElementById("stapel").addEventListener("click", zieheKarte);
function zieheKarte() {
    if (kartenStapel.length > 0) {
        let k;
        k = Math.floor((Math.random() * kartenStapel.length));
        let prodElement = document.createElement('div');
        document.getElementById("kartenHand").appendChild(prodElement);
        kartenHand.push(kartenStapel[k]);
        let karteziehen = `<div class="${kartenStapel[k].Typ}"> ${kartenStapel[k].Typ} ${kartenStapel[k].Wert} </div>`;
        prodElement.innerHTML = karteziehen;
        kartenStapel.splice(k, 1);
        document.getElementById("kartenHand").innerHTML = "";
        for (let v = 0; v < kartenHand.length; v++) {
            writeHtml(v);
        }
    }
    else {
        alert("KEINE KARTEN MEHR AUF DEM STAPEL");
    }
}
//Zug (Passende Karte ablegen)
document.getElementById("kartenHand").addEventListener("click", karteAblegen);
function karteAblegen(event) {
    let s = 0;
    let i = 0;
    while (s < kartenHand.length) {
        let karteId = event.target;
        console.log(kartenHand.length);
        if (ablagestapel.length > 3 && kartenHand.length == 0) {
            alert("Du hast gewonnen!");
            console.log("Du hast gewonnen!");
        }
        else if (ablagestapel.length > 3 && compHand.length == 0) {
            alert("Das tut mir Leid! Du hast verloren!");
            console.log("Du hast verloren!");
        }
        if (kartenHand[s].Reihenfolge == Number(karteId.getAttribute("id"))) {
            if (kartenHand[s].Typ == ablagestapel[ablagestapel.length - 1].Typ || kartenHand[s].Wert == ablagestapel[ablagestapel.length - 1].Wert) {
                ablagestapel.push(kartenHand[s]);
                let prodElement = document.createElement('div');
                document.getElementById("ablagestapel").appendChild(prodElement);
                let ablegekarte = `<div class="${kartenHand[s].Typ}" id="${kartenHand[s].Reihenfolge}"> ${kartenHand[s].Wert}</div>`;
                prodElement.innerHTML = ablegekarte;
                kartenHand.splice(s, 1);
                document.getElementById("kartenHand").innerHTML = "";
                for (let o = 0; o < kartenHand.length; o++) {
                    writeHtml(o);
                    VariableX = kartenHand[s];
                }
                computerGegner(VariableX);
            }
            i += 1;
        }
        s++;
    }
}
let VariableX;
if (kartenHand.length > 0 && ablagestapel.length > 4) {
    alert("Glückwunsch! Du hast gewonnen!");
}
function computerGegner(_meineKarte) {
    let s = 0;
    for (let he = 0; he < compHand.length; he++) {
        if (compHand[he].Typ == ablagestapel[ablagestapel.length - 1].Typ || compHand[he].Wert == ablagestapel[ablagestapel.length - 1].Wert) {
            let prodElement = document.createElement('div');
            document.getElementById("ablagestapel").appendChild(prodElement);
            let ablegekarte = `<div class="${compHand[he].Typ}" id="${compHand[he].Reihenfolge}"> ${compHand[he].Wert}</div>`;
            prodElement.innerHTML = ablegekarte;
            compHand.splice(he, 1);
            var times = 1;
            for (var i = 0; i < times; i++) {
                doSomething();
            }
            function doSomething() {
                document.getElementById("Computer").innerHTML = "";
            }
        }
        writeHtml2(s);
    }
    {
    }
}
//# sourceMappingURL=TS-END.js.map